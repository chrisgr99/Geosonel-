 ...
 *
 *   note("c4 e4 g4 c5").gain(imageLightness.range(0.2, 1.0))
 *
 ...
 *
 * For Phase 4's first commit only imageLightness lands;
 * the rest of the OKLCh-derived signal set (imageChroma,
 * imageLum, imageRedness, imageGreenness, imageYellowness,
 * imageBlueness) follows in close succession since each
 * is a trivial wrapper once the precomputed OKLCh buffer
 * in canvas.js and the firing-context-pointer plumbing
 * are in place.
 *
 ...
 * imageLightness parse correctly once the engine is up.
 */
...
/**
 * The compiled Pattern for imageLightness, populated by
 * installImageSignals. Module-level so a single Pattern
 * instance is shared across all references; null before
 * registration so an early access path can detect the
 * "engine not loaded yet" state.
 * @type {any | null}
 */
let imageLightnessPattern = null;

/**
 * Read the imageLightness value for the currently-firing
 * source from the active firing context. Returns 0 when no
 ...
 * loaded (imageOKLCh is null). Zero is the chosen no-data
 * default because it maps cleanly through .range(lo, hi)
 * to the low end of the destination range, which is a
 * sensible musical default (quiet, low pitch, or neutral
 * value depending on the operator the composer reaches
 * for).
 *
 * @returns {number}
 */
function readImageLightness() {
 ...
 * the signals unbound. Patterns using imageLightness
 * would then fail to parse with a clear ReferenceError
 * rather than silently emitting 0 forever.
 */
export function installImageSignals() {
    ...
    if (typeof signalFn !== "function") {
        console.warn(
            ...
            "referencing imageLightness will fail to parse.",
        );
        return;
    }
    ...
    imageLightnessPattern = signalFn((_t) => readImageLightness());
    /** @type {any} */ (window).imageLightness = imageLightnessPattern;
    console.log("[signals] imageLightness installed");
}
