...
import { GUITAR_SAMPLES, GUITAR_SOUND_NAMES } from "./guitarSamples.js";
...
export class StrudelRuntime {
    ...
    ensureSamplesForVoice(sound, bank) {
        ...
        if (typeof sound === "string") {
            if (VCSL_SOUND_NAMES.has(sound)) {
                ...
                this._ensureInlineSamples(sound, BASS_SAMPLES[sound]);
            } else if (GUITAR_SOUND_NAMES.has(sound)) {
                this._ensureInlineSamples(sound, GUITAR_SAMPLES[sound]);
    ...
    /**
     * Register one in-code multisampled instrument's note-to-URL map (a bass or a
     * guitar) with strudel, exactly once per session. Unlike the VCSL / drum-
     * machine maps (a single hosted JSON fetched by URL), each instrument is a
     * small in-code map passed to samples() as an OBJECT — { soundName: { "A1":
     * url, ... } } with absolute jsDelivr URLs — so no JSON is fetched here; only
     * the per-note audio lazy-loads on first play. Deduped under a synthetic
     * "inst:<name>" key in the same set the URL loads use; a failed registration
     * un-marks it so a later pick can retry.
     *
     * @param {string} sound  The instrument sound name, e.g. "swag_bass".
     * @param {any} map  Its note-to-URL map (BASS_SAMPLES[sound] / GUITAR_SAMPLES[sound]).
     */
    _ensureInlineSamples(sound, map) {
        if (map === undefined || map === null) return;
        const key = "inst:" + sound;
        ...
        Promise.resolve(samplesFn({ [sound]: map })).then(
            () => console.log(`${LOG_PREFIX} lazy-loaded samples: ${sound}`),
            (err) => {
                this._loadedSampleUrls.delete(key);
                console.warn(`${LOG_PREFIX} lazy sample load failed: ${sound}`, err);
            },
        );
    }
