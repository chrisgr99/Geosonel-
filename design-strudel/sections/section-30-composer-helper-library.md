...
A score's harmony — its key and chord progression — is defined by the cascade model in Section 11: it lives on the Shared baseline and cascades to segments, the key authored as a field and the progression as a `$chords` / `$chords@<segment>` labelled block. The helper library adds no separate score-level harmony system. What it offers is a convenience: when a callback or a declarative voice passes key, scale, and progression into a helper, those fields can be grouped in a plain options object, shown here as `scoreHarmony`:
