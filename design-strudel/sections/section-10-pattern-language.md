...
Harmony-context signals expose the active harmony at the firing source: `currentScale`, `currentChord`, `currentTonic`, `currentRoot`. The active harmony is the effective key (tonal centre and scale) and chord progression, which cascade from a Shared baseline to segments, each segment inheriting or overriding either (Section 11); @strudel/tonal is now in scope as the mechanism that resolves the active scale degrees into concrete pitches. The signals are not yet implemented, but the integration is committed rather than deferred.
