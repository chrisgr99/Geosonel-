...
This section captures the in-progress design of a composer helper library for GXW and the authoring idioms that support it. The library covers signal-transform helpers, scale and chord helpers in both declarative pattern-returning and procedural single-value forms, MIDI note emission with voice management, the callback context shape used by procedural helpers, and the editor affordances that support all of the above. The design is iterating in this section while it stabilizes. As individual pieces settle and implementations land, the canonical documentation for each piece migrates to its natural section: Section 9 (Behaviour Slots and behaviors.js) for the callback API, Section 11 (Musical Structure) for harmony, Section 19 (Audio and MIDI Output) for MIDI output and voice management, Section 28 (Pattern Authoring and the Cursor Model) for editor affordances. This section also accumulates worked examples and eventually condenses to a recipes collection or becomes a forwarding stub once the design is fully absorbed, following the model used by Section 28 after the v2.5 fold-back pass.

Status. The pieces of this section's design that have shipped are the mapClip transform helper plus the MIDISender fix to accept numeric MIDI notes (commit 1 of the in-flight signal-range-control series), and the playNote / playSound procedural emitters described below — the immediate-fire substrate, shipped as positional functions ahead of the options-object voice layer that will sit above them. Everything else in this section is design-in-progress: shapes agreed in conversation but not yet implemented. Open design questions are flagged in the Future Scope subsection at the end rather than scattered through the body.
...
A score's harmony — its key and chord progression — is defined by the cascade model in Section 11: it lives on the Shared baseline and cascades to sections, the key authored as a field and the progression as a `$chords` / `$chords@<section>` labelled block. The helper library adds no separate score-level harmony system. What it offers is a convenience: when a callback or a declarative voice passes key, scale, and progression into a helper, those fields can be grouped in a plain options object, shown here as `scoreHarmony`:
...
The authoritative home of the progression is Section 11's cascading `$chords` labelled block, not this options object. Section 11's section model has absorbed the old harmony-conductor idea — form gating became per-section object state, and chord-change tokens became the cascading key and chord progression — so there is no separate conductor mechanism to reconcile against. The options-object form here is purely a helper-API convenience for passing the effective harmony into a scaleNotes or chordNotes call; it reads the same key and progression the cascade resolves rather than defining a parallel one.
...
### Procedural note and sound emission

The first procedural emitters to ship are playNote and playSound, the immediate-fire primitives a callback invokes to sound a single note or sample the moment it runs. They are exposed on the onTick context today (Section 9); the hasHit and beenHit collision callbacks, their primary intended home, expose the same two methods once those call sites are wired (the collision firing path is a separate later piece). They are the substrate the voice-managed midiNote helper described below will sit on, shipped on their own first so note emission from a callback can be exercised before the voice model lands.

These two emitters take positional arguments rather than the options object the rest of the library uses. The departure is deliberate and scoped to this layer: a callback firing one note is the high-frequency, low-ceremony case, and a short positional call reads cleanly inside a per-event body where the handful of values — what to play, how loud, how long — are few enough to hold in order. The options-object pattern still governs the higher-level helpers (scaleNotes, chordNotes, and the planned midiNote voice layer), where the parameter count is larger and base-plus-override reuse matters; the reconciliation of the two forms is recorded as an open question in Future Scope.

playNote(sound, note, amplitude, duration, articulation) fires a pitched note. sound is the superdough instrument name (e.g. "piano", "marimba"), ignored on the MIDI path where the channel selects the instrument. note is a MIDI note number or a note-name string; both output paths accept either. amplitude is a 0-to-1 level, read as gain on superdough and as velocity on MIDI. duration is the total wall-clock window the note occupies, tail included, in seconds (defaulting to a short value when omitted, since a procedural note is not beat-quantized and so has no musical division to inherit). articulation is the gate: how long the note holds at strength before damping begins, defaulting to the full duration.

The duration-and-articulation pair is the absolute-seconds form of the clip articulation concept. On superdough the gate is the held length and the release tail fills the remainder, so a note of duration 1.0 and articulation 0.3 strikes, holds 0.3 s, then damps across the remaining 0.7 s and ends at 1.0 s; the note always sums back to its total duration. The decay and sustain come from the per-instrument envelope table (Section 19, the firing engine's VOICE_ENVELOPES); only the release is derived from the articulation. With articulation omitted the gate is the full duration and the release falls back to the table's short default, which reproduces an ordinary pattern note's articulation. On the MIDI path the note-off fires at the gate and the external synth supplies its own release tail, so the beyond-gate portion is the synth's to shape; notes go out on channel 1 (a dedicated MIDI-only emitter exposing port and channel is deferred).

playSound(bank, sample, amplitude) fires a sample. bank is the drum-machine kit name (e.g. "RolandTR909"; omit it for the default dirt-samples kit), sample is the sample name (e.g. "bd"), and amplitude is the 0-to-1 gain. The bank resolves a raw sample name against the kit at superdough lookup time, mirroring strudel's .bank() operator; a pre-banked name already carrying an underscore is left alone. playSound is silent under the MIDI output by design — a raw drum sample carries no MIDI note, so there is nothing to send; percussion through MIDI is played with playNote and a General-MIDI percussion note number instead. It is a one-shot with no synthetic envelope, since drum samples carry their own decay, unlike the sustained pitched instruments playNote shapes.

Both emitters dispatch into the same output layer the cyclePatterns use, so a callback note sounds identical to that object's pattern notes, and both fire the moment the callback runs rather than quantizing to a beat — the value of a procedural note is precisely that its timing follows the environment as the object moves. Each dispatches a few milliseconds ahead of the audio clock's current time rather than exactly at it, so superdough has the scheduling headroom Web Audio needs and does not drop the occasional note. Each also requests the resolved instrument or bank's sample map through the runtime's lazy loader, so an instrument named in a call (a marimba, say) sounds without first having to be selected in the inspector; the very first hit on a freshly-named instrument can be silent while its map fetches, then sounds from then on.

A per-source rate limiter guards against the tick-rate flood an ungated onTick call would otherwise produce. Each object may fire at most one procedural note or sample per minimum interval — initially a 64th note at 120 BPM (about 31 ms), a starting value to tune by ear — and the cap is shared across playNote and playSound, so it bounds the object's total audio messages rather than each kind separately; a tick that calls both fires only the first. The throttle is gated on the deterministic simulation clock and reset on rewind and on a home-teleport alongside the per-object random state, so a replay fires identically. It is a safety floor, not a substitute for authoring: the intended use is collision callbacks, which fire once per collision, and an author firing from onTick is still expected to gate the call themselves (a threshold crossing, an every-N-beats counter) rather than lean on the throttle. These primitives carry none of the voice management — the mono modes, the minimum-duration gate, the per-object polyphony cap — described in the next subsection; that layer is the deferred midiNote design.

### MIDI note emission with voice management

The playNote and playSound primitives above fire immediately but carry no voice management. This subsection describes the voice-managed layer that sits above them, designed (but not yet implemented) as the midiNote helper. Whether that layer keeps the options-object form midiNote(opts) shown here or adopts a positional form consistent with playNote, and whether it wraps the shipped primitives or supersedes them, is an open question recorded in Future Scope.
...
clip sets articulation, borrowing strudel's clip as a multiplier on how long the note actually sounds: the note fires immediately and its note-off, or its envelope release on the superdough path, lands at the fire time plus duration times clip. A clip of one sounds the full duration, below one is staccato, above one overruns into a legato overlap. On superdough, clip maps to the native clip parameter; on MIDI it is realised by placing the note-off at the scaled time. Because the note-off and the voice-slot release coincide, a staccato note frees its voice slot early and a legato one holds it longer. The level a note carries — its velocity, in MIDI terms — is read as gain on the superdough path, one slot interpreted as velocity or amplitude depending on the active output. The note's pitch is a MIDI note number, which is exactly what the procedural scale helper (scaleNote, above) returns, so a scale lookup drops straight into the note call with no conversion.

A per-object polyphony cap is the voice policy designed for onTick-fired notes, which arrive immediately and at environment-driven intervals rather than on a grid. Each object carries a maximum number of simultaneously sounding notes; when onTick requests a note while the object is already at that cap, the request is dropped, and the next request after a sounding note expires gets through. Unlike the mono modes, the cap never forces an older note off, so it behaves identically on MIDI and on superdough — superdough has no note-off handle to steal a voice with, but a drop-when-full policy needs none. The characteristic to expect is first-come-holds: a long duration paired with a chatty onTick drops many requests, which thins dense passages on its own. The cap, the mono modes, and minDuration are three independent levers on density — the cap bounds simultaneous voices, the mono modes bound a scope to one voice with stealing, and minDuration paces re-articulation from a single source — and a score may combine them; folding them into one coherent voice model is recorded as an open question in Section 25.
...
This Score tab is a structured editing surface for the score's voices and voice management, parallel to the Code, Properties, and Canvas tabs in the editor pane. (The score's key and chord progression are structure, authored in Section 11's own Score tab; the two are distinct surfaces that happen to share a name, and folding them together is a later question.) It serves two related purposes. For users without an AI assistant integrated into their workflow (the default case, since GXW is free and AI integration is not part of the app), the Score tab is the primary surface for authoring voice parameters. For users with AI access who edit primarily through natural-language instructions, the Score tab is also a structured reading surface that makes the current state of the score easy to scan and zoom into without reading raw JSON.
...
The bucket structure covers the voice-authoring surface. The score's key and chord progression are not a bucket here — they are score structure, authored in Section 11's Score tab (the key as a field on the Shared and section cards, the progression as a `$chords` labelled block), where a progression's mini-notation can carry a live chord-name preview (so "I V vi [I IV]" shows "C G Am [C F]" in C major). A Voices section lists every sound-producing object in the score as its own bucket, with the voice's identifier at the top and the fields it carries as rows beneath. A Shared Mappings bucket lists named modulation specs defined at score scope (velocityFromChroma, durationFromR, and so on), each with its source signal and output range summarized. A Voice Management bucket shows groups and which voices belong to each, with their voice-mode summaries collected in one place. A Conductors bucket (or a separate Conductors tab in evolved versions) lists conductor curves and their patterns, distinct from the voices list because conductors produce score-orchestration events rather than notes.
...
Positional emitters versus the options-object library. The shipped playNote and playSound emitters take positional arguments, a deliberate departure from the options-object pattern the rest of the library follows, scoped to the high-frequency single-note callback case. The voice-managed emission layer is still designed as midiNote(opts) in the options-object form. Whether that layer keeps the options object or adopts a positional form consistent with the shipped emitters, and whether it wraps playNote / playSound or supersedes them, is unsettled; the resolution waits on building the voice model and seeing how a voice-managed call reads at the use site. A plausible end state is that playNote / playSound remain the bare positional primitives and the voice-managed helper is a distinct, options-object call that an author reaches for when they want mono modes, a minimum-duration gate, or a polyphony cap.

A runtime harmony-state concept. The `scoreHarmony` options object described above is effectively a static harmony state that callers explicitly read. A runtime harmony state, a property on the score (or on individual curves) that helpers read by default when their opts object omits the harmony fields, would let callbacks be even more compact. The trade-off is that hidden defaults are less legible at the use site, and the existing options-object pattern with spread overrides already covers most reuse cases. This idea is not adopted in the current design but is noted as a possible evolution.
...
Relationship to Section 11's cascading harmony. The `scoreHarmony` options object covers the helper-API convenience of grouping key, scale, and progression for a call. The authoritative progression is Section 11's cascading `$chords` labelled block, and the old harmony-conductor mechanism has been absorbed into the section model. The open question is the binding between the two: whether the helpers read the cascade's effective harmony automatically (so a callback need not restate it) or continue to take it explicitly through the options object. The runtime-harmony-state idea above is one route to the automatic form.
