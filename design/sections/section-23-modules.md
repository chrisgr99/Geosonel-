...
7. Scene — holds image, regions, curves, triggers, sprites, and score-level parameters (tempo, time signature). Harmony is per-section, carried on each section rather than at the score level.
...
11. Phrase — deprecated at v2.5; superseded by PatternEngine (module 6). The Phrase module name reflected an earlier design where time-indexed event sequences were a first-class concept; in the current model patterns are Strudel Pattern values evaluated by PatternEngine and emitted directly without an intermediate Phrase representation.
