...
| Active Beats string | 5 | 1 | Schema has `activeBeats: beatsString` on curves. Read binding wired (curves only). |
| Beat Strength string | 5 | 1 | Schema has `strength: strengthString, default "9"` on curves. Read binding wired (curves only). |
| Sprite Auto Beats / Strength | 5 | 3 | Sprites have no active-beats or strength fields in the schema, and design Section 6 specifies a simple auto-timer interval, not a beat pattern. Inspector v1 activates Band 5 for sprites with the label "Sprite Auto Beats", anticipating a model extension that has not yet been agreed. Decided for now: Band 5 read binding consults curves only; sprites activate the band in the layout but contribute nothing to the displayed values. When the sprite-auto-timer simulation milestone arrives, extend the Sprite class with `activeBeats` and `strength` fields, add them to `SPRITE_FIELDS`, and update the aggregate sources in `_buildBandBeatPoints` to include `objs.sprites`. Tracked here so the sprite-side wiring isn't overlooked. |
| Cycle Speeds | 6 | 1 | Schema has `cycleSpeeds: string, default "1"` on curves. Space-separated multipliers cycling through the list cycle by cycle, negative values reverse direction. Read binding wired. |
| Stop at Cycle | 6 | 1 | Schema has `stopAtCycle: integer, default -1` on curves. -1 means play forever; positive integers halt the cursor after that many full cycles. Read binding wired. |
