...
Callbacks take **no parameters**. Inside the body, `this` is bound to the firing object's context: `this.vel` / `this.velocity` (default note velocity 0–1; in `onActiveBeat` this is the beat accent rescaled), `this.beatStrength` (`onActiveBeat` only — the crossed beat's Beat-Strength digit 0–9 rescaled to 0–1), `this.col.*` (colour signals under the firing point — `r`, `g`, `b`, `y`, `or`, `li`, `cy`, `pu`, `lt`, `chr`), plus other per-slot fields. Sound is emitted with **bare** emitter calls (no `this.` prefix): `playNote(note, vel?, dur?, pan?)`, `playSound(sample, vel?)`, `applyForce(...)`. `playNote("xylophone", 60)` with a leading string overrides the instrument. The pattern-emission API is described in `DESIGN.md` Section 9 (Behaviour Slots) and Section 10 (Pattern Language). Read those before authoring substantial behaviour code.
